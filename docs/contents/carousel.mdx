...
### External Control with useCarousel

```tsx
import { useCarousel } from '@/components/ui/carousel';

function MyComponent() {
  const { currentIndex, goToSlide, goToNext, goToPrevious } = useCarousel(5);

  return (
    <View>
      <Button onPress={goToPrevious}>Previous</Button>
      <Button onPress={goToNext}>Next</Button>
      <Button onPress={() => goToSlide(2)}>Go to slide 3</Button>

      <Carousel onIndexChange={goToSlide}>{/* Your carousel items */}</Carousel>
    </View>
  );
}
```
...
- **External Control**: useCarousel hook for programmatic control
